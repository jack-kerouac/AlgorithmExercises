...
/**
 * <P>
 * This class adds easy right-justified output (formatting) and alignment of the
 * decimal separator to the existing capabilities of java.text.DecimalFormat and
 * its parent class, java.text.NumberFormat.
 * </P>
 * <P>
 * Create a formatter/parser using one of the factory methods of the form,
 * getXXXInstance( int decPlaces, Locale loc). Specify the maximum number of
 * decimal places your program will need, across all locales where it will be
 * run. Make (instantiate) a separate formatter object for each format type
 * (Number, Currency, or Percent). Use Locale.getDefault() to localize the
 * formatting and parsing to whatever a user's particular language and country
 * may be. Or you can specify the Locale variables and then display output for
 ...
 * </P>
 * <P>
 * The following additions are provided to what is already available from the
 * core java.text.DecimalFormat class:<BR>
 * <UL>
 * <LI>format( double dval, int fieldsize) -- Output will be right-aligned to
 * the decimal point using the default (space) or whatever pad character you
 * specify with setPadCharacter(char padChar).
 * <LI>setPattern( String pat ) and getPattern() -- Methods override the
 * applyPattern( String pat ) and toPattern() methods in DecimalFormat so you
 * can use the pattern string as a JavaBean property.
 * <LI>setMaxFieldSize( int size ) and getMaxFieldSize() -- control the
 * maxFieldSize property. The maxFieldSize is the largest fieldsize your
 * monospaced program output needs. Default is 80. Use the set()er method if you
 * need something larger.
 * <LI>setFractionDigits( int digits ) and getFractionDigits() -- The set()er
 * sets <EM>both</EM> the maximum and minimum number of fraction digits. The
 * get()er gets the current minimum number of fraction digits, regardless of
 * whether setFractionDigits() has been previously called.
 ...
 * <BR>
 * <P>
 * For compatibility with code you may have written already, all three standard
 * DecimalFormat constructors are provided. But to really take advantage of this
 * class's raison d'&ecirc;tre, use one of the getXXXInstance factory
 * constructors wherever you can. The standard constructors allow you to use
 * PaddedDecimalFormat wherever you have used DecimalFormat in your existing
 * code.
 * </P>
 * <P>
...
public class PaddedDecimalFormat extends DecimalFormat {
	...
	/**
	 * Mimics the usual DecimalFormat constructor, allowing you to use this
	 * class everywhere your existing code uses DecimalFormat.
	 * 
	 * @see #getNumberInstance(int, java.util.Locale)
	 * @see #getPercentInstance(int, java.util.Locale)
	 * @see #getCurrencyInstance(int, java.util.Locale)
	...
	/**
	 * The full-function DecimalFormat constructor. Included here for use by the
	 * getXXXInstance() methods which follow. It is public simply to provide
	 * complete compatibility with existing DecimalFormat code.
	 * 
	 * @see #getNumberInstance(int, java.util.Locale)
	 * @see #getPercentInstance(int, java.util.Locale)
	 * @see #getCurrencyInstance(int, java.util.Locale)
	 ...
	 * 
	 * @param decPlaces
	 *            Number of decimal places to plan for. Make this the maximum
	 *            number of decimal places your application will use, and keep
	 *            its value constant across all the formatters you may
	 *            instantiate.
	 * @param loc
	 *            The Locale (language and country combination).
	 */
	public static PaddedDecimalFormat getNumberInstance(int decPlaces,
			Locale loc) {
		DecimalFormat temp = (DecimalFormat) NumberFormat
				.getNumberInstance(loc);
	...
	/**
	 * Right-justified double-to-String conversion. Default behavior is to
	 * return decimal aligned decimal strings, as long as the fieldsize is large
	 * enough.
	 * 
	 * @param dval
	 *            a double (small "d") value. Returns an error message for
	 *            values larger than the maximum (smaller than the minimum) Long
	 *            integer.
	 * @param fieldsize
	 *            desired number of monospaced characters, including padding.
	 *            The fieldsize must be less than the maxFieldSize, which
	 *            defaults to 80.
	...
	/**
	 * Set the maximum fieldsize: useful if you need more than the default value
	 * of 80.
	 */
	...
	/**
	 * Set <EM>both</EM> the maximum and minimum number of fraction digits to be
	 * displayed.
	 * 
	 * @param places
	 *            Values can be 0 to the decPlaces value used to create this
	 *            formatting object. Specify -1 to indicate you want the default
	 *            currency format's maximum number of decimal places.
	 */
	public void setFractionDigits(int places) {
		if (places == -1) {
			if (defaultCurPlaces == -1) { // case where this var has never been
											// set
	...
	/**
	 * Control whether alignment will occur at the decimal separator or at the
	 * last numeric digit.
	 * 
	 * @param alignField
	 *            Can be NumberFormat.INTEGER_FIELD (the default) which causes
	 *            alignment of decimal separators, or
	 *            NumberFormat.FRACTION_FIELD which causes alignment of the last
	 *            numeric digit.
	 */
	...
	/**
	 * Attempt to change this formatting object by supplying a new DecimalFormat
	 * pattern string. Throws IllegalArgumentException if the attempt fails. For
	 * example, switching the decimal separator to be a grouping separator will
	 * fail. Use a new formatting object rather than setPattern() to switch
	 * among locales.<BR>
	 * This method uses DecimalFormat's applyPattern() method. I have not found
	 * the applyLocalizedPattern() method useful.
	 */
	...
	/**
	 * Attempt to change this formatting object by supplying a new DecimalFormat
	 * pattern string. Throws IllegalArgumentException if the attempt fails.<BR>
	 * Uses DecimalFormat's applyLocalizedPattern() method and provided here for
	 * "completeness." I haven't found it useful.
	 */
	 ...
	 * needed locale (language/country combination). To cause formatting to
	 * localize for any particular user's locale, call the constructor with
	 * Locale.getDefault() as the Locale variable.
	 */
 ...
 * TUuScT2E4w/gk7BQeZ46F8daiJHW9w8i0mpZk2J+9BX3AfiroL3gUFNnpN4I5AQl 1o5gk+TQ/14=
 * =/ets -----END PGP SIGNATURE-----
 */
